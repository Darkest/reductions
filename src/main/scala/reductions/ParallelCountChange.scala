...
object ParallelCountChange {
  ...
  def parCountChange(money: Int, coins: List[Int], threshold: Threshold): Int = {
    if (money == 0) 1 else if (money < 0) 0 else if (coins.isEmpty) 0 else if (threshold(money, coins))
      countChange(money - coins.head, coins) + countChange(money, coins.tail)
    else {
  ...
  def moneyThreshold(startingMoney: Int): Threshold = (money: Int, coins: List[Int]) =>
    if (money <= startingMoney * 2 / 3) true else false

  /** Threshold heuristic based on the total number of initial coins. */
  def totalCoinsThreshold(totalCoins: Int): Threshold =(money: Int, coins: List[Int]) =>
    if (coins.length <= totalCoins * 2 / 3) true else false
